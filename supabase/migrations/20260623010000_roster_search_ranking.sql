-- Add relevance ranking to the grouped roster search.
-- Before: results were ordered subject→catalog alphabetically, so a substring
-- hit like "CS 11" matching "BCS 1131" (bcs1131 contains cs11) sorted ABOVE the
-- actual CS courses. Now rank by match quality first:
--   0 exact code | 1 code prefix | 2 code substring | 3 title-only
-- then fall back to subject/catalog order.
